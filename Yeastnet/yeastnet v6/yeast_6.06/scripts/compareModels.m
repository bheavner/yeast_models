...
%              This code currently compares the first 2 models.
%
%   results (optional)
...
if ~exist('results','var')
    for index=1:length(models)
        results(index) = testYeast(models(index), 0, 0, 1, 0);
    end
end
...
for index=1:length(results)
    ...
    per_model(index).biomass = results(index).biomass;
    ...
    if isfield(models(index),'blocked')
...
comparison.unique_auxotrophs_1 = setdiff(results(1).auxotrophs,results(2).auxotrophs);
comparison.unique_auxotrophs_2 = setdiff(results(2).auxotrophs,results(1).auxotrophs);
...
comparison.common_correct_auxotrophs = intersect(correct_auxotrophs_1,correct_auxotrophs_2);
comparison.unique_correct_auxotrophs_1 = setdiff(correct_auxotrophs_1,correct_auxotrophs_2);
comparison.unique_correct_auxotrophs_2 = setdiff(correct_auxotrophs_2,correct_auxotrophs_1);
comparison.common_auxotrophs_viable = intersect(results.auxotroph_viable);
comparison.unique_auxotrophs_viable_1 = setdiff(results(1).auxotroph_viable,results(2).auxotroph_viable);
comparison.unique_auxotrophs_viable_2 = setdiff(results(2).auxotroph_viable,results(1).auxotroph_viable);
comparison.common_auxotrophs_inviable = intersect(results.auxotroph_inviable);
comparison.unique_auxotrophs_inviable_1 = setdiff(results(1).auxotroph_inviable,results(2).auxotroph_inviable);
comparison.unique_auxotrophs_inviable_2 = setdiff(results(2).auxotroph_inviable,results(1).auxotroph_inviable);

%compare constraints
rxnIndexes1 = findRxnIDs(models(1),comparison.common_rxns);
rxnIndexes2 = findRxnIDs(models(2),comparison.common_rxns);

same_lb_indexes = rxnIndexes1(models(1).lb(rxnIndexes1) ==  models(2).lb(rxnIndexes2));
differing_lb_indexes = rxnIndexes1(~(models(1).lb(rxnIndexes1) ==  models(2).lb(rxnIndexes2)));
...
same_ub_indexes = rxnIndexes1(models(1).ub(rxnIndexes1) ==  models(2).ub(rxnIndexes2));
differing_ub_indexes = rxnIndexes1(~(models(1).ub(rxnIndexes1) ==  models(2).ub(rxnIndexes2)));
