function bh_testYeastModel(filename,verbose)
...
%% create model with kuepfer medium + glucose %%TODO

model_kuepfer_glu = model;
ExchangeRxnNames = {
...
ind = ismember(model_kuepfer_glu.rxnNames,ExchangeRxnNames);
model_kuepfer_glu.ub(ind) = 1000;
model_kuepfer_glu.lb(ind) = -1000;

%% create model with kuepfer medium + galactose %%TODO

model_kuepfer_gal = model_kuepfer_glu;
ind = ismember(model_kuepfer_gal.rxnNames,'D-glucose exchange');
model_kuepfer_gal.ub(ind) = 0;
model_kuepfer_gal.lb(ind) = -1000;

ind = ismember(model_kuepfer_gal.rxnNames,'D-galactose exchange');
model_kuepfer_gal.ub(ind) = 10;
model_kuepfer_gal.lb(ind) = -1000;

%% create model with kuepfer medium + glycerol %%TODO

model_kuepfer_gly = model_kuepfer_glu;
ind = ismember(model_kuepfer_gly.rxnNames,'D-glucose exchange');
model_kuepfer_gly.ub(ind) = 0;
model_kuepfer_gly.lb(ind) = -1000;

ind = ismember(model_kuepfer_gly.rxnNames,'glycerol exchange');
model_kuepfer_gly.ub(ind) = 10;
model_kuepfer_gly.lb(ind) = -1000;

%% create model with kuepfer medium + ethanol %%TODO

model_kuepfer_eth = model_kuepfer_glu;
ind = ismember(model_kuepfer_eth.rxnNames,'D-glucose exchange');
model_kuepfer_eth.ub(ind) = 0;
model_kuepfer_eth.lb(ind) = -1000;

ind = ismember(model_kuepfer_eth.rxnNames,'ethanol exchange');
model_kuepfer_eth.ub(ind) = 10;
model_kuepfer_eth.lb(ind) = -1000;
...
%minimal media, using SGD essential and auxotroph annotation (note: SGD
%uses YPD for essential annotation, so this is a bit apples and oranges) -
%expect deletions of essential and auxotroph genes to cause inviable
%phenotype
...
exp_retarded = intersect(exp_retarded,verifiedORFs); %or is it better to leave unverified ORFs in other models?

exp_viable = setdiff(model.genes,inviableORFsAll);
exp_viable = intersect(exp_viable,verifiedORFs);%or is it better to leave unverified ORFs in other models?

grRatio = singleGeneDeletion(model);
%grRatio = singleGeneDeletion(model_maximal); %bh changed to use same essential def.
...
disp('min media SGD essential + auxotroph knockout analysis (positive = viable):');
...
fprintf('%g%%\t%s\n%g%%\t%s\n%g%%\t%s\n%g%%\t%s\n%',...
    ...
    roundSF(100*n_tp/(n_tp+n_fp)),'positive predictive value = precision = tp/(tp+fp)',...
    roundSF(100*n_tn/(n_fn+n_tn)),'negative predictive value = tn/(fn+tn)'...
    );

% fprintf('g%%\t%s\n%g%%\t%s\n',...
%     roundSF(100*(n_tn+n_tp)/n_genes),'accuracy = (tp+tn)/(tp+tn+fp+fn)',...
%     roundSF(100*2*(n_tp/(n_tp+n_fn) * n_tp/(n_tp+n_fp))/(n_tp/(n_tp+n_fn) + n_tp/(n_tp+n_fp))),'F-measure = 2.(precision.recall)/(precision+recall)'...
%     );
...
%maximal media, using SGD essential annotation - expect deletions of
%essential genes to cause inviable phenotype, deletion of auxotrophs should
%be viable

% do not combine inviable with auxotrophy list
...
disp('max media SGD essential knockout analysis (positive = viable):');
...
% kuepfer media, + glucose aerobic - expect to match kuepfer essential
% annotation -- for now, just Y5 media changes

% do not combine inviable with auxotrophy list
inviableORFsAll = kuepfer_glucose_aerobic;

exp_retarded = intersect(model_kuepfer_glu.genes,inviableORFsAll);
exp_retarded = intersect(exp_retarded,verifiedORFs); %or is it better to leave unverified ORFs in other models?

exp_viable = setdiff(model_kuepfer_glu.genes,inviableORFsAll);
exp_viable = intersect(exp_viable,verifiedORFs);%or is it better to leave unverified ORFs in other models?

%grRatio = singleGeneDeletion(model);
grRatio = singleGeneDeletion(model_kuepfer_glu);

mod_viable  = model_kuepfer_glu.genes(grRatio >= ko_tol);
mod_viable = intersect(mod_viable,verifiedORFs);
mod_retarded = model_kuepfer_glu.genes(grRatio < ko_tol);
mod_retarded = intersect(mod_retarded,verifiedORFs);

disp('kuepfer media + glucose knockout analysis (positive = viable):');
...
n_genes = length(intersect(model_kuepfer_glu.genes,verifiedORFs)); %#ok<NASGU>
...
% kuepfer media, + galactose - expect to match kuepfer essential annotation

% do not combine inviable with auxotrophy list
inviableORFsAll = kuepfer_galactose_aerobic;

exp_retarded = intersect(model_kuepfer_gal.genes,inviableORFsAll);
exp_retarded = intersect(exp_retarded,verifiedORFs); %or is it better to leave unverified ORFs in other models?

exp_viable = setdiff(model_kuepfer_gal.genes,inviableORFsAll);
exp_viable = intersect(exp_viable,verifiedORFs);%or is it better to leave unverified ORFs in other models?

%grRatio = singleGeneDeletion(model);
grRatio = singleGeneDeletion(model_kuepfer_gal);

mod_viable  = model_kuepfer_gal.genes(grRatio >= ko_tol);
mod_viable = intersect(mod_viable,verifiedORFs);
mod_retarded = model_kuepfer_gal.genes(grRatio < ko_tol);
mod_retarded = intersect(mod_retarded,verifiedORFs);

disp('kuepfer media + galactose knockout analysis (positive = viable):');
...
% kuepfer media, + glycerol - expect to match kuepfer essential annotation

% do not combine inviable with auxotrophy list
inviableORFsAll = kuepfer_glycerol_aerobic;

exp_retarded = intersect(model_kuepfer_gly.genes,inviableORFsAll);
exp_retarded = intersect(exp_retarded,verifiedORFs); %or is it better to leave unverified ORFs in other models?

exp_viable = setdiff(model_kuepfer_gly.genes,inviableORFsAll);
exp_viable = intersect(exp_viable,verifiedORFs);%or is it better to leave unverified ORFs in other models?

%grRatio = singleGeneDeletion(model);
grRatio = singleGeneDeletion(model_kuepfer_gly);

mod_viable  = model_kuepfer_gly.genes(grRatio >= ko_tol);
mod_viable = intersect(mod_viable,verifiedORFs);
mod_retarded = model_kuepfer_gly.genes(grRatio < ko_tol);
mod_retarded = intersect(mod_retarded,verifiedORFs);
...
% kuepfer media, + ethanol - expect to match kuepfer essential annotation
% do not combine inviable with auxotrophy list
inviableORFsAll = kuepfer_ethanol_aerobic;

exp_retarded = intersect(model_kuepfer_eth.genes,inviableORFsAll);
exp_retarded = intersect(exp_retarded,verifiedORFs); %or is it better to leave unverified ORFs in other models?

exp_viable = setdiff(model_kuepfer_eth.genes,inviableORFsAll);
exp_viable = intersect(exp_viable,verifiedORFs);%or is it better to leave unverified ORFs in other models?

%grRatio = singleGeneDeletion(model);
grRatio = singleGeneDeletion(model_kuepfer_eth);

mod_viable  = model_kuepfer_eth.genes(grRatio >= ko_tol);
mod_viable = intersect(mod_viable,verifiedORFs);
mod_retarded = model_kuepfer_eth.genes(grRatio < ko_tol);
mod_retarded = intersect(mod_retarded,verifiedORFs);

disp('kuepfer media + ethanol knockout analysis (positive = viable):');
...
%could also do kuepfer glucose anaerobic...
...
%% Gene lists - what is essential? Which mutations cause auxotrophies?

function genes = inviableORFs

% list of 1191 unique essential ORFs taken from the Yeast Deletion Project (14 aug
% 11)
...
genes={'YAL021C';'YAL024C';'YAL026C';'YAL040C';'YAL051W';'YAL056W';'YAL058W';'YAR003W';'YAR015W';'YAR069W-A';'YAR070W-A';'YBL027W';'YBL033C';'YBL047C';'YBL058W';'YBL061C';'YBL091C-A';'YBL098W';'YBL102W';'YBL103C';'YBR015C';'YBR058C';'YBR077C';'YBR106W';'YBR107C';'YBR115C';'YBR126C';'YBR127C';'YBR133C';'YBR175W';'YBR176W';'YBR189W';'YBR191W';'YBR248C';'YBR272C';'YBR279W';'YCL018W';'YCL030C';'YCL032W';'YCL033C';'YCL045C';'YCR021C';'YCR045C';'YCR047C';'YCR053W';'YCR076C';'YCR089W';'YCR094W';'YDL001W';'YDL002C';'YDL006W';'YDL010W';'YDL020C';'YDL021W';'YDL033C';'YDL040C';'YDL048C';'YDL069C';'YDL073W';'YDL074C';'YDL077C';'YDL081C';'YDL083C';'YDL106C';'YDL130W';'YDL131W';'YDL173W';'YDL190C';'YDL191W';'YDL192W';'YDL194W';'YDL201W';'YDL203C';'YDR007W';'YDR043C';'YDR049W';'YDR057W';'YDR071C';'YDR074W';'YDR080W';'YDR138W';'YDR162C';'YDR173C';'YDR174W';'YDR176W';'YDR200C';'YDR226W';'YDR260C';'YDR266C';'YDR276C';'YDR277C';'YDR283C';'YDR289C';'YDR335W';'YDR346C';'YDR348C';'YDR351W';'YDR354W';'YDR358W';'YDR363W';'YDR379W';'YDR385W';'YDR389W';'YDR392W';'YDR395W';'YDR411C';'YDR422C';'YDR432W';'YDR439W';'YDR448W';'YDR469W';'YDR477W';'YDR482C';'YDR486C';'YDR540C';'YEL004W';'YEL013W';'YEL021W';'YEL027W';'YEL029C';'YEL031W';'YEL037C';'YEL040W';'YEL044W';'YEL048C';'YEL051W';'YER001W';'YER007C-A';'YER026C';'YER027C';'YER052C';'YER055C';'YER059W';'YER069W';'YER083C';'YER090W';'YER091C';'YER092W';'YER095W';'YER101C';'YER116C';'YER118C';'YER120W';'YER122C';'YER129W';'YER130C';'YER149C';'YER150W';'YER152C';'YER167W';'YER169W';'YER177W';'YFL013C';'YFL031W';'YFR010W';'YFR019W';'YFR025C';'YFR040W';'YFR047C';'YFR048W';'YGL009C';'YGL012W';'YGL020C';'YGL025C';'YGL026C';'YGL031C';'YGL049C';'YGL054C';'YGL058W';'YGL060W';'YGL062W';'YGL066W';'YGL070C';'YGL115W';'YGL126W';'YGL127C';'YGL154C';'YGL167C';'YGL168W';'YGL175C';'YGL179C';'YGL180W';'YGL181W';'YGL203C';'YGL211W';'YGL219C';'YGL234W';'YGL244W';'YGR014W';'YGR020C';'YGR056W';'YGR057C';'YGR061C';'YGR063C';'YGR092W';'YGR104C';'YGR105W';'YGR108W';'YGR135W';'YGR144W';'YGR162W';'YGR166W';'YGR204W';'YGR223C';'YGR227W';'YGR229C';'YGR241C';'YGR250C';'YGR252W';'YHR010W';'YHR013C';'YHR018C';'YHR025W';'YHR026W';'YHR030C';'YHR038W';'YHR060W';'YHR079C';'YHR111W';'YHR142W';'YHR162W';'YHR178W';'YHR179W';'YHR199C';'YHR206W';'YHR208W';'YIL017C';'YIL020C';'YIL027C';'YIL029C';'YIL036W';'YIL044C';'YIL072W';'YIL077C';'YIL105C';'YIL116W';'YIL119C';'YIL128W';'YIL153W';'YIR017C';'YIR034C';'YJL088W';'YJL095W';'YJL115W';'YJL117W';'YJL128C';'YJL130C';'YJL140W';'YJL151C';'YJL153C';'YJL158C';'YJL172W';'YJL192C';'YJL193W';'YJL204C';'YJL208C';'YJR066W';'YJR075W';'YJR083C';'YJR104C';'YJR122W';'YKL001C';'YKL006W';'YKL027W';'YKL032C';'YKL048C';'YKL053C-A';'YKL056C';'YKL064W';'YKL077W';'YKL079W';'YKL119C';'YKL121W';'YKL160W';'YKL176C';'YKL190W';'YKL211C';'YKL212W';'YKL213C';'YKL216W';'YKR007W';'YKR026C';'YKR036C';'YKR070W';'YKR099W';'YLL019C';'YLL021W';'YLL027W';'YLL039C';'YLR015W';'YLR016C';'YLR021W';'YLR048W';'YLR055C';'YLR061W';'YLR074C';'YLR079W';'YLR087C';'YLR113W';'YLR150W';'YLR192C';'YLR199C';'YLR226W';'YLR242C';'YLR262C';'YLR268W';'YLR292C';'YLR315W';'YLR320W';'YLR324W';'YLR332W';'YLR357W';'YLR371W';'YLR373C';'YLR396C';'YLR417W';'YLR418C';'YLR420W';'YLR426W';'YLR436C';'YML008C';'YML013W';'YML014W';'YML028W';'YML034W';'YML055W';'YML071C';'YML103C';'YML115C';'YML117W';'YMR010W';'YMR014W';'YMR016C';'YMR029C';'YMR038C';'YMR052W';'YMR062C';'YMR067C';'YMR068W';'YMR092C';'YMR099C';'YMR104C';'YMR123W';'YMR165C';'YMR190C';'YMR202W';'YMR214W';'YMR217W';'YMR242C';'YMR247C';'YMR276W';'YMR300C';'YMR304W';'YMR307W';'YMR312W';'YNL003C';'YNL041C';'YNL051W';'YNL079C';'YNL080C';'YNL119W';'YNL127W';'YNL133C';'YNL148C';'YNL215W';'YNL219C';'YNL220W';'YNL229C';'YNL236W';'YNL241C';'YNL277W';'YNL307C';'YNL316C';'YNL322C';'YNR055C';'YOL018C';'YOL067C';'YOL087C';'YOL090W';'YOL093W';'YOL098C';'YOL107W';'YOL109W';'YOL111C';'YOL116W';'YOL121C';'YOL122C';'YOL124C';'YOL143C';'YOL145C';'YOR002W';'YOR008C';'YOR012W';'YOR067C';'YOR070C';'YOR078W';'YOR096W';'YOR106W';'YOR123C';'YOR128C';'YOR189W';'YOR202W';'YOR216C';'YOR246C';'YOR290C';'YOR320C';'YOR322C';'YOR359W';'YOR371C';'YPL055C';'YPL065W';'YPL089C';'YPL138C';'YPL140C';'YPL144W';'YPL157W';'YPL159C';'YPL174C';'YPL177C';'YPL226W';'YPL241C';'YPL254W';'YPL264C';'YPR036W';'YPR043W';'YPR060C';'YPR067W';'YPR139C';'YPR167C';'YPR173C';'YPR179C';'YPR201W';};
...
genes = {'YDR300C';'YJR122W';'YAL012W';'YER068C-A';'YHR018C';'YAR015W';'YBL033C';'YBR115C';'YDR007W';'YDR127W';'YDR158W';'YDR234W';'YDR354W';'YEL024W';'YER052C';'YER086W';'YER090W';'YGL026C';'YGL148W';'YGL154C';'YGL234W';'YGR061C';'YGR204W';'YHR025W';'YIL094C';'YIR034C';'YJL088W';'YJR139C';'YKL211C';'YLR304C';'YML022W';'YMR300C';'YNL220W';'YNL316C';'YOL058W';'YOL143C';'YOR128C';'YPR060C';'YBR209W';'YCR066W';'YDL072C';'YDL075W';'YDR008C';'YDR477W';'YFL032W';'YGR168C';'YHL005C';'YKR058W';'YPL006W';'YPR067W';'YGR155W';'YCR053W';'YNR050C';'YIL069C';'YKL135C';'YER057C';'YER069W';'YHL003C';'YNL218W';'YDR523C';'YOR184W';'YMR135W-A';};

function genes = kuepfer_galactose_aerobic

% list of 311 genes found to be essential in defined media + galactose +
% aerobic
%Doi: 10.1101/gr.3992505 

genes = {'YDR300C';'YJR122W';'YAL012W';'YER068C-A';'YAR015W';'YBL033C';'YBR115C';'YDR007W';'YDR127W';'YDR158W';'YDR234W';'YDR354W';'YEL024W';'YER052C';'YER086W';'YER090W';'YGL026C';'YGL148W';'YGL154C';'YGL234W';'YGR061C';'YGR204W';'YHR025W';'YIL094C';'YIR034C';'YJL088W';'YJR139C';'YKL211C';'YLR304C';'YML022W';'YMR300C';'YNL220W';'YNL316C';'YOL058W';'YOL143C';'YOR128C';'YPR060C';'YBR209W';'YCR066W';'YDL072C';'YDL075W';'YDR008C';'YDR477W';'YFL032W';'YGR168C';'YHL005C';'YKR058W';'YPL006W';'YPR067W';'YGR155W';'YCR053W';'YNR050C';'YIL069C';'YKL135C';'YER057C';'YDR523C';'YOR184W';'YMR135W-A';'YOR130C';'YHR177W';'YHR177W';'YBL045C';'YBL099W';'YBR003W';'YDL067C';'YDR298C';'YDR529C';'YEL024W';'YER070W';'YGL012W';'YGR180C';'YGR255C';'YHR051W';'YLR295C';'YML110C';'YMR256C';'YMR267W';'YNR041C';'YOL096C';'YOR065W';'YOR125C';'YOR241W';'YPL059W';'YPL078C';'YPL078C';'YPL148C';'YPL271W';'YAL009W';'YAL039C';'YAL048C';'YBL002W';'YBL012C';'YBL022C';'YBL038W';'YBL090W';'YBL100C';'YBR037C';'YBR112C';'YBR120C';'YBR122C';'YBR132C';'YBR179C';'YBR251W';'YBR268W';'YBR282W';'YCR003W';'YCR004C';'YCR024C';'YCR028C-A';'YCR046C';'YCR071C';'YDL044C';'YDL045W-A';'YDL049C';'YDL057W';'YDL062W';'YDL063C';'YDL068W';'YDL069C';'YDL104C';'YDL107W';'YDL113C';'YDL135C';'YDL146W';'YDL167C';'YDL181W';'YDL198C';'YDL202W';'YDR042C';'YDR065W';'YDR078C';'YDR079W';'YDR114C';'YDR194C';'YDR197W';'YDR204W';'YDR230W';'YDR231C';'YDR237W';'YDR268W';'YDR283C';'YDR295C';'YDR296W';'YDR322W';'YDR337W';'YDR507C';'YDR518W';'YEL029C';'YEL036C';'YEL050C';'YER017C';'YER050C';'YER058W';'YER087W';'YER103W';'YER110C';'YER122C';'YER141W';'YER153C';'YER154W';'YER169W';'YFL016C';'YFL036W';'YGL038C';'YGL064C';'YGL095C';'YGL107C';'YGL129C';'YGL135W';'YGL218W';'YGL220W';'YGL240W';'YGR062C';'YGR076C';'YGR102C';'YGR112W';'YGR150C';'YGR171C';'YGR215W';'YGR219W';'YGR220C';'YGR222W';'YGR257C';'YHL038C';'YHR011W';'YHR038W';'YHR091C';'YHR116W';'YHR120W';'YHR147C';'YHR168W';'YIR021W';'YJL003W';'YJL063C';'YJL096W';'YJL102W';'YJL180C';'YJL209W';'YJR004C';'YJR144W';'YKL003C';'YKL134C';'YKL138C';'YKL155C';'YKL169C';'YKL170W';'YKL194C';'YKR057W';'YKR061W';'YKR085C';'YLL018C-A';'YLR067C';'YLR069C';'YLR091W';'YLR114C';'YLR139C';'YLR202C';'YLR203C';'YLR204W';'YLR238W';'YLR255C';'YLR257W';'YLR312W-A';'YLR369W';'YML061C';'YML088W';'YML129C';'YMR024W';'YMR064W';'YMR066W';'YMR071C';'YMR072W';'YMR084W';'YMR089C';'YMR097C';'YMR098C';'YMR150C';'YMR151W';'YMR158W';'YMR184W';'YMR193W';'YMR228W';'YMR257C';'YMR282C';'YMR286W';'YMR287C';'YNL003C';'YNL005C';'YNL073W';'YNL081C';'YNL084C';'YNL160W';'YNL170W';'YNL177C';'YNL184C';'YNL213C';'YNL225C';'YNL252C';'YNL284C';'YNR036C';'YNR037C';'YOL023W';'YOL095C';'YOL100W';'YOR033C';'YOR150W';'YOR158W';'YOR199W';'YOR200W';'YOR201C';'YOR205C';'YOR211C';'YOR304C-A';'YOR305W';'YOR318C';'YPL005W';'YPL013C';'YPL029W';'YPL031C';'YPL040C';'YPL097W';'YPL104W';'YPL118W';'YPL132W';'YPL172C';'YPL173W';'YPL215W';'YPR047W';'YPR072W';'YPR099C';'YPR100W';'YPR124W';'YPR166C';'YOR187W';'YKL016C';'YCR084C';'YDR115W';'YDR175C';'YMR231W';'YPR116W';'YGL206C';'YGR262C';'YGL223C';'YLR382C';'YMR021C';'YBR018C';'YBR019C';'YBR020W';'YBR020W';'YDR009W';'YPL248C';};

function genes = kuepfer_glycerol_aerobic

% list of 305 genes found to be essential in defined media + glycerol +
% aerobic
%Doi: 10.1101/gr.3992505 

genes = {'YDR300C';'YJR122W';'YAL012W';'YAR015W';'YBL033C';'YBR115C';'YDR007W';'YDR127W';'YDR158W';'YDR234W';'YDR354W';'YEL024W';'YER052C';'YER086W';'YER090W';'YGL026C';'YGL148W';'YGL154C';'YGL234W';'YGR061C';'YGR204W';'YHR025W';'YIL094C';'YIR034C';'YJL088W';'YJR139C';'YKL211C';'YLR304C';'YML022W';'YMR300C';'YNL220W';'YNL316C';'YOL058W';'YOL143C';'YOR128C';'YPR060C';'YBR209W';'YCR066W';'YDL072C';'YDL075W';'YDR008C';'YDR477W';'YFL032W';'YGR168C';'YHL005C';'YKR058W';'YPL006W';'YPR067W';'YGR155W';'YCR053W';'YDR523C';'YOR184W';'YMR135W-A';'YOR130C';'YHR177W';'YHR177W';'YBL045C';'YBL099W';'YBR003W';'YDL067C';'YDR298C';'YDR529C';'YEL024W';'YER070W';'YGL012W';'YGR180C';'YGR255C';'YHR051W';'YLR295C';'YML110C';'YMR256C';'YMR267W';'YNR041C';'YOL096C';'YOR065W';'YOR125C';'YOR241W';'YPL059W';'YPL078C';'YPL078C';'YPL148C';'YPL271W';'YAL009W';'YAL039C';'YAL048C';'YBL002W';'YBL012C';'YBL022C';'YBL038W';'YBL090W';'YBL100C';'YBR037C';'YBR112C';'YBR120C';'YBR122C';'YBR132C';'YBR179C';'YBR251W';'YBR268W';'YBR282W';'YCR003W';'YCR004C';'YCR024C';'YCR028C-A';'YCR046C';'YCR071C';'YDL044C';'YDL045W-A';'YDL049C';'YDL057W';'YDL062W';'YDL063C';'YDL068W';'YDL069C';'YDL104C';'YDL107W';'YDL113C';'YDL135C';'YDL146W';'YDL167C';'YDL181W';'YDL198C';'YDL202W';'YDR042C';'YDR065W';'YDR078C';'YDR079W';'YDR114C';'YDR194C';'YDR197W';'YDR204W';'YDR230W';'YDR231C';'YDR237W';'YDR268W';'YDR283C';'YDR295C';'YDR296W';'YDR322W';'YDR337W';'YDR507C';'YDR518W';'YEL029C';'YEL036C';'YEL050C';'YER017C';'YER050C';'YER058W';'YER087W';'YER103W';'YER110C';'YER122C';'YER141W';'YER153C';'YER154W';'YER169W';'YFL016C';'YFL036W';'YGL038C';'YGL064C';'YGL095C';'YGL107C';'YGL129C';'YGL135W';'YGL218W';'YGL220W';'YGL240W';'YGR062C';'YGR076C';'YGR102C';'YGR112W';'YGR150C';'YGR171C';'YGR215W';'YGR219W';'YGR220C';'YGR222W';'YGR257C';'YHL038C';'YHR011W';'YHR038W';'YHR091C';'YHR116W';'YHR120W';'YHR147C';'YHR168W';'YIR021W';'YJL003W';'YJL063C';'YJL096W';'YJL102W';'YJL180C';'YJL209W';'YJR004C';'YJR144W';'YKL003C';'YKL134C';'YKL138C';'YKL155C';'YKL169C';'YKL170W';'YKL194C';'YKR057W';'YKR061W';'YKR085C';'YLL018C-A';'YLR067C';'YLR069C';'YLR091W';'YLR114C';'YLR139C';'YLR202C';'YLR203C';'YLR204W';'YLR238W';'YLR255C';'YLR257W';'YLR312W-A';'YLR369W';'YML061C';'YML088W';'YML129C';'YMR024W';'YMR064W';'YMR066W';'YMR071C';'YMR072W';'YMR084W';'YMR089C';'YMR097C';'YMR098C';'YMR150C';'YMR151W';'YMR158W';'YMR184W';'YMR193W';'YMR228W';'YMR257C';'YMR282C';'YMR286W';'YMR287C';'YNL003C';'YNL005C';'YNL073W';'YNL081C';'YNL084C';'YNL160W';'YNL170W';'YNL177C';'YNL184C';'YNL213C';'YNL225C';'YNL252C';'YNL284C';'YNR036C';'YNR037C';'YOL023W';'YOL095C';'YOL100W';'YOR033C';'YOR150W';'YOR158W';'YOR199W';'YOR200W';'YOR201C';'YOR205C';'YOR211C';'YOR304C-A';'YOR305W';'YOR318C';'YPL005W';'YPL013C';'YPL029W';'YPL031C';'YPL040C';'YPL097W';'YPL104W';'YPL118W';'YPL132W';'YPL172C';'YPL173W';'YPL215W';'YPR047W';'YPR072W';'YPR099C';'YPR100W';'YPR124W';'YPR166C';'YOR187W';'YIL125W';'YLR377C';'YBL044W';'YDR470C';'YOR158W';'YOL002C';'YER068C-A';'YGL206C';'YGR262C';'YNL315C';'YOL017W';'YOR221C';'YFL018C';'YHR067W';'YNR006W';'YOR330C';};

function genes = kuepfer_ethanol_aerobic

% list of 331 genes found to be essential in defined media + ethanol +
% aerobic
%Doi: 10.1101/gr.3992505 

genes = {'YDR300C';'YJR122W';'YAR015W';'YBL033C';'YBR115C';'YDR007W';'YDR127W';'YDR158W';'YDR234W';'YDR354W';'YEL024W';'YER052C';'YER086W';'YER090W';'YGL026C';'YGL148W';'YGL154C';'YGL234W';'YGR061C';'YGR204W';'YHR025W';'YIL094C';'YIR034C';'YJL088W';'YJR139C';'YKL211C';'YLR304C';'YML022W';'YMR300C';'YNL220W';'YNL316C';'YOL058W';'YOL143C';'YOR128C';'YPR060C';'YBR209W';'YCR066W';'YDL072C';'YDL075W';'YDR008C';'YDR477W';'YFL032W';'YGR168C';'YHL005C';'YKR058W';'YPL006W';'YPR067W';'YDR523C';'YMR135W-A';'YHR177W';'YHR177W';'YBL045C';'YBL099W';'YBR003W';'YDL067C';'YDR298C';'YDR529C';'YEL024W';'YER070W';'YGL012W';'YGR180C';'YGR255C';'YHR051W';'YLR295C';'YML110C';'YMR256C';'YMR267W';'YNR041C';'YOL096C';'YOR065W';'YOR125C';'YOR241W';'YPL059W';'YPL078C';'YPL078C';'YPL148C';'YPL271W';'YAL009W';'YAL039C';'YAL048C';'YBL002W';'YBL012C';'YBL022C';'YBL038W';'YBL090W';'YBL100C';'YBR037C';'YBR112C';'YBR120C';'YBR122C';'YBR132C';'YBR179C';'YBR251W';'YBR268W';'YBR282W';'YCR003W';'YCR004C';'YCR024C';'YCR028C-A';'YCR046C';'YCR071C';'YDL044C';'YDL045W-A';'YDL049C';'YDL057W';'YDL062W';'YDL063C';'YDL068W';'YDL069C';'YDL104C';'YDL107W';'YDL113C';'YDL135C';'YDL146W';'YDL167C';'YDL181W';'YDL198C';'YDL202W';'YDR042C';'YDR065W';'YDR078C';'YDR079W';'YDR114C';'YDR194C';'YDR197W';'YDR204W';'YDR230W';'YDR231C';'YDR237W';'YDR268W';'YDR283C';'YDR295C';'YDR296W';'YDR322W';'YDR337W';'YDR507C';'YDR518W';'YEL029C';'YEL036C';'YEL050C';'YER017C';'YER050C';'YER058W';'YER087W';'YER103W';'YER110C';'YER122C';'YER141W';'YER153C';'YER154W';'YER169W';'YFL016C';'YFL036W';'YGL038C';'YGL064C';'YGL095C';'YGL107C';'YGL129C';'YGL135W';'YGL218W';'YGL220W';'YGL240W';'YGR062C';'YGR076C';'YGR102C';'YGR112W';'YGR150C';'YGR171C';'YGR215W';'YGR219W';'YGR220C';'YGR222W';'YGR257C';'YHL038C';'YHR011W';'YHR038W';'YHR091C';'YHR116W';'YHR120W';'YHR147C';'YHR168W';'YIR021W';'YJL003W';'YJL063C';'YJL096W';'YJL102W';'YJL180C';'YJL209W';'YJR004C';'YJR144W';'YKL003C';'YKL134C';'YKL138C';'YKL155C';'YKL169C';'YKL170W';'YKL194C';'YKR057W';'YKR061W';'YKR085C';'YLL018C-A';'YLR067C';'YLR069C';'YLR091W';'YLR114C';'YLR139C';'YLR202C';'YLR203C';'YLR204W';'YLR238W';'YLR255C';'YLR257W';'YLR312W-A';'YLR369W';'YML061C';'YML088W';'YML129C';'YMR024W';'YMR064W';'YMR066W';'YMR071C';'YMR072W';'YMR084W';'YMR089C';'YMR097C';'YMR098C';'YMR150C';'YMR151W';'YMR158W';'YMR184W';'YMR193W';'YMR228W';'YMR257C';'YMR282C';'YMR286W';'YMR287C';'YNL003C';'YNL005C';'YNL073W';'YNL081C';'YNL084C';'YNL160W';'YNL170W';'YNL177C';'YNL184C';'YNL213C';'YNL225C';'YNL252C';'YNL284C';'YNR036C';'YNR037C';'YOL023W';'YOL095C';'YOL100W';'YOR033C';'YOR150W';'YOR158W';'YOR199W';'YOR200W';'YOR201C';'YOR205C';'YOR211C';'YOR304C-A';'YOR305W';'YOR318C';'YPL005W';'YPL013C';'YPL029W';'YPL031C';'YPL040C';'YPL097W';'YPL104W';'YPL118W';'YPL132W';'YPL172C';'YPL173W';'YPL215W';'YPR047W';'YPR072W';'YPR099C';'YPR100W';'YPR124W';'YPR166C';'YIL125W';'YLR377C';'YBL044W';'YDR470C';'YOR158W';'YKL016C';'YCR084C';'YDR115W';'YDR175C';'YMR231W';'YPR116W';'YER068C-A';'YGL206C';'YGR262C';'YNR050C';'YIL069C';'YKL135C';'YGL223C';'YLR382C';'YMR021C';'YHR018C';'YER069W';'YHL003C';'YLR308W';'YDR226W';'YDR377W';'YER065C';'YGR183C';'YGR208W';'YKL148C';'YKR097W';'YOL126C';'YPL262W';'YPR191W';'YBL021C';'YEL027W';'YGL218W';'YGL237C';'YHR039C-B';'YLR396C';'YNL001W';'YNL315C';'YOL148C';'YOR381W';'YGL252C';'YOR187W';'YOR221C';'YOR302W';};
